...
declare global {
  interface Window {
    ...
    syncMraidMediaState?: () => void;
    setSipPresentationActive?: (active: boolean) => void;
    setSipHostMediaAllowed?: (allowed: boolean) => void;
...
export function initMraid(): Promise<void> {
  ...
  const attach = (): void => {
    ...
    guarded('audioVolumeChange', (value: number) => {
      if (typeof value !== 'number') return;
      audioVolumePercentage = value;
      window.dispatchEvent(new Event('mraidaudiovolumechange'));
    });
  };
  attach();
...
export function getAudioVolume(): number {
  return Math.min(100, Math.max(0, audioVolumePercentage)) / 100;
}
export function isAdViewable(): boolean { return !pauseRequired(); }
export function triggerCTA(storeUrl: string): void {
  notifyGameClose();
  // Preserve a click target supplied by the AppLovin host while STORE_URL is
  // intentionally pending PM confirmation.
  if (storeUrl) window.clickTag = storeUrl;
